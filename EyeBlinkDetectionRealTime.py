...
class Message:
    ...
    def startTimer(self):  # 초 세고 종료시키는 함수
        ...
        if (self.running):  # 실행중이면
            ...
            if (self.timer > 600):  # 제한시간동안 팝업 확인 안하면  ms
...
class GUI:  # 변수 초기화 및 포맷
    ...
    def open_file(self):  # 인강과 웹캠 여는 함수
        ...
        print('long:%s' % long)
        self.time=np.random.randint(int(long)*20,int(long)*60)
        print(self.time)
    def get_frame(self):
        try:
            ...
            self.cursor.execute("UPDATE BUCKET SET attendance ='o' WHERE b_stid='%s' and b_coursenum='%s'" % (DB_ID, c_num))
            ...
            while not self.pause:  # 영상 중지 아니면
                ...
                if ret:
                    ...
                    for rect in rects:  # 웹캠 단위 쪼개서 분석 시작
                        ...
                        if self.UP_COUNTER > 100:  # 눈 계속 떴을때 10초
                            # 팝업 열기전 프레임 초기화
                            self.UP_COUNTER = 0
                            self.DOWN_COUNTER = 0
                            Message(self,1)
                        ...
                        if(int(self.time)//60==int(self.timer2)/10):
                            print(self.time / 60)
                            print(self.timer2 / 10)
                            Message(self,2)
